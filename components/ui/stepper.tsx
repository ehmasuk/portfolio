...
import { Check, Spinner } from "@hugeicons/core-free-icons";
...
const Stepper = React.forwardRef<HTMLDivElement, StepperProps>(({ defaultValue = 0, value, onValueChange, orientation = "horizontal", className, ...props }, ref) => {
...
const StepperItem = React.forwardRef<HTMLDivElement, StepperItemProps>(({ step, completed = false, disabled = false, loading = false, className, children, ...props }, ref) => {
...
const StepperTrigger = React.forwardRef<HTMLDivElement, StepperTriggerProps>(({ asChild = false, className, children, ...props }, ref) => {
  ...
  return (
    <div ref={ref} className={cn("flex w-full items-center gap-6 disabled:pointer-events-none disabled:opacity-50", className)} onClick={() => setActiveStep(step)} {...props}>
      {children}
    </div>
  );
});
        ...
        <>
          ...
          {isLoading && (
            <span className="absolute transition-all">
              <HugeiconsIcon icon={Spinner} size={14} className="animate-spin" strokeWidth={2} aria-hidden="true" />
            </span>
          )}
        </>
      )}
...
// StepperTitle
const StepperTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(({ className, ...props }, ref) => (
...
// StepperSeparator
const StepperSeparator = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
