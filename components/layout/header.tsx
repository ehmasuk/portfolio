  ...
  return (
    <div className="relative w-full mx-auto border-x-[0.01rem] overflow-hidden rounded border-gray-200 dark:border-white/5 flex gap-4 items-center justify-end py-14 px-6">
      <motion.div
        ...
        className="flex gap-4 items-center bg-background px-2 rounded border-[0.01rem] border-gray-200 dark:border-white/5"
      >
        <Link href="/" className={cn("font-mono hover:text-primary", pathname !== "/" && "text-muted-foreground")}>
          Profile
        </Link>
        <Link href="/blog" className={cn("font-mono hover:text-primary", !pathname.startsWith("/blog") && "text-muted-foreground")}>
          Blogs
        </Link>
        <Link href="/" className={cn("font-mono hover:text-primary", !pathname.startsWith("/components") && "text-muted-foreground")}>
          Components
        </Link>
        <ThemeToggler />
      </motion.div>
